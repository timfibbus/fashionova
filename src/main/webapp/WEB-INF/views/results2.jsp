		...
		<form>
  			<img name="image" src="${result.thumbnail }" class="card-img-top" >
  			 <div class="card-body">
  			 	...
  			 	<p class="card-text"> Rating:  <c:out value="${result.rating }"/>/5 </p>
  			 	</c:if>
  			 	<c:if test="${result.reviews > 0 }" > 
  			 	<p class="card-text"> Reviews:  <c:out value="${result.reviews }"/> </p>
  			 	</c:if>
  			 	<p class="card-text">  <c:out value="${result.title }"/> </p>
